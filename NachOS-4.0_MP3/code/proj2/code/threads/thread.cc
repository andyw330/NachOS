...
Thread::Thread(char* threadName, int threadID, int threadPri)
{
	ID = threadID;
    name = threadName;
    priority = threadPri;
    ...
    stack = NULL;
    status = JUST_CREATED;
	cout << "Statistics:\tProcessNew\t" << ID << "\t" << kernel->stats->totalTicks << endl;
...
Thread::~Thread()
{
    DEBUG(dbgThread, "Deleting thread: " << name);
   	//cout << "thread is deleted " << name << endl;
...
Thread::Finish ()
{
	...
	cout << "Statistics:\tProcessFinish\t" << kernel->currentThread->getID() << "\t" << kernel->stats->totalTicks << endl;
...
Thread::Yield ()
{
	...
	kernel->scheduler->changePriority();
...
/*void
Thread::MultiLevel_Yield (List<Thread *> *readyList)
{
	Thread *nextThread;
	IntStatus oldLevel = kernel->interrupt->SetLevel(IntOff);
...
Thread::Sleep (bool finishing)
{
	...
	cout << "Statistics:\tProcessSleep\t" << kernel->currentThread->getID() << "\t" << kernel->stats->totalTicks << endl;
    status = BLOCKED;
...
Thread::SelfTest()
{
    ...
    Thread *t = new Thread("forked thread", 1, 75);
...
/*
Semaphore* getSemaphore(int threadID)
{
	Thread* t = kernel->getThread(threadID);
	return kernel->s;
}
*/
