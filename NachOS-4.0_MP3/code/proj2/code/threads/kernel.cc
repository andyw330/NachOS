...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
		...
		//			cout << "test ID " << initTime[i] << " name " << jobName[i] << endl;
				}	
				fp.close();
			}	
...
Kernel::Initialize()
{
	...
	PageTs = new Ptable[1024];
	for(int i=0;i < 1024; i++)   ///初始化，memory
        	PageTs[i].Phy=0;
    ...
    alarm = new Alarm(randomSlice, 200);	// start up time slicing
    ...
    currentThread = new Thread("main", threadNum++,75);		
...
{
	cout << "name num is " << name << ",threadNum: " << threadNum << ",threadPri: " << priority[threadNum] <<endl;;
	fflush(stdout);
