...
// advance simulated time
	...
	while (kernel->initTime[kernel->listCounter] < stats->totalTicks && kernel->listCounter < kernel->totalList) {
//		cout << "listCounter totalTicks " << kernel->listCounter << ", " << stats->totalTicks << endl;
		kernel->listCounter++;
//		cout << "Name is " << kernel->jobName[kernel->listCounter-1] << endl;
		kernel->Exec(kernel->jobName[kernel->listCounter-1]);
	}
...
void
Interrupt::Nice(int ID){

   Thread *a = kernel->getThread(ID);
   //kernel->scheduler->readyList->Remove(a);
   a->setPri(a->getPri() - 20);
   //kernel->scheduler->ReNew(a);
   cout << "thread: " << a->getName() << " change priority to " << a->getPri() <<"\n";
}
...
Interrupt::Schedule(CallBackObj *toCall, int fromNow, IntType type)
{
	...
	if (fromNow<=0) cout << "fromNow is " << fromNow << endl; 
    ASSERT(fromNow > 0);
...
Interrupt::CheckIfDue(bool advanceClock)
{
    ...
    if (pending->IsEmpty()) {   	// no pending interrupts
		//cout << "Debug in Interrupt::CheckIfDue, pending is empty\n";
		return FALSE;	
    }		
    next = pending->Front();
	//cout << "Debug in Interrupt::CheckIfDue, next is " << intTypeNames[next->type] << " and when is " << next->when << " total is " << stats->totalTicks << endl;
