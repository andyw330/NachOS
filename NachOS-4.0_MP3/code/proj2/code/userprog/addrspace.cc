...
AddrSpace::~AddrSpace()
{
	Ptable *buf = kernel->PageTs ;
	for(int i = 0; i< numPages; i++){
		buf[pageTable[i].physicalPage].Phy=0; 
	}
   delete pageTable;
}
...
// how big is address space?
	...
	Ptable *buf = kernel->PageTs ;  //產生一個table來記錄physical page
...
// Note: this code assumes that virtual address = physical address
	...
	if (noffH.code.size > 0) {
		...
		cout << "test:" << noffH.code.virtualAddr << ", " << noffH.code.size;
		cout << "page is " << pageTable[noffH.code.virtualAddr/PageSize].physicalPage << endl;
		cout << "address is " << pageTable[noffH.code.virtualAddr/PageSize].physicalPage*PageSize+(noffH.code.virtualAddr%PageSize) << endl;
        executable->ReadAt(
...
AddrSpace::Execute(char *fileName) 
{
	...
	//cout << "debug AddrSpace::Execute() before machine->Run()\n";
    kernel->machine->Run();		// jump to the user progam
	//cout << "debug AddrSpace::Execute() after  machine->Run()\n";
...
{
	//cout << "debug AddrSpace::RestoreState()\n";
    kernel->machine->pageTable = pageTable;
    kernel->machine->pageTableSize = numPages;
	if (pageTable==NULL && kernel->machine->pageTable==NULL) cout << "WTF!!\n";	
}
